...
include 'connect2.php';
  ...
  $num_per_page = 02;
	...
	$sql="SELECT * FROM bill limit $start_from,$num_per_page";
...
<!--table to display data-->
    <table>
      ...
      //     echo "<tr><td>".$row['bill_id'] ."</td><td>". $row["item_id"]."</td><td>". $row["cus_id"]."</td><td>". $row["bill_quantity"]."</td><td>". $row["bill_price"]."</td><td>". $row["user_id"] . "</td></tr>";
      //   };
      //   echo "</table>";
      // }
      ...
      <td><?php echo $rows['bill_id'];?></td>
            ...
            <td><?php echo $rows['bill_quantity'];?></td>
            <td><?php echo $rows['bill_price'];?></td>
    ...
    <?php
    $pr_sql="SELECT * FROM bill";
    $pr_result= mysqli_query($conn,$pr_sql);
    $total_records=mysqli_num_rows($pr_result);
    $total_pages=ceil($total_records/$num_per_page);
    
    if($page>1){
      echo "<a href = 'payments2.php?page=".($page-1)."' class='btn btn-danger'> Previous </a>";
    }

    for($i=1;$i<=$total_pages;$i++)
    {
        echo "<a href='payments2.php?page=".$i."' class ='btn btn-primary'>".$i."</a>" ;
    }

    if($i>$page){
      echo "<a href = 'payments2.php?page=".($page+1)."' class='btn btn-danger'>Next</a>";
    }
